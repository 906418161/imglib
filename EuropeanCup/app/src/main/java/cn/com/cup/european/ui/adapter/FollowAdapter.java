...
public class FollowAdapter extends BaseAdapter implements RequestCallbackListener {
    ...
    public FollowAdapter(List<FollowBean> beans, Context context) {
        this.beans = beans;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        FollowBean bean = beans.get(position);
        ImageSelectUtil.showImg(viewHolder.head, bean.getHeadimg());
        viewHolder.fans.setText("粉丝:" + bean.getFans());
        viewHolder.name.setText(bean.getName());
        viewHolder.sig.setText(bean.getSignature());
        if (bean.getType().equals("0")) {
            viewHolder.v.setVisibility(View.GONE);
            viewHolder.follow.setImageResource(bean.getIsLike().equals("0")
                    ? R.drawable.people_follow : R.drawable.people_follow_yes);
        } else {
        ...
        viewHolder.follow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                beans.get(position).setFans(bean.getIsLike().equals("0") ?
                        String.valueOf(Integer.parseInt(bean.getFans()) - 1) :
                        String.valueOf(Integer.parseInt(bean.getFans()) + 1));
